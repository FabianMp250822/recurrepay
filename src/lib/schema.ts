...
export const publicClientSchema = z.object({
  ...
  financingPlan: z.coerce.number({invalid_type_error: "Debe seleccionar un plan de financiación."})
    .refine(val => typeof val === 'number', { message: "Seleccione un plan de financiación válido."}), // Ensures a plan is selected
  paymentDayOfMonth: z.coerce.number().int().min(1, { message: "El día debe estar entre 1 y 31." }).max(31, { message: "El día debe estar entre 1 y 31." }),
  // paymentAmount será calculado, no ingresado por el cliente directamente si hay contrato.
  // Si contractValue es 0, entonces paymentAmount debe ser > 0 (para servicios recurrentes sin contrato)
}).refine(data => {
  if (data.contractValue > 0 && data.financingPlan === undefined) {
    return false; // Si hay valor de contrato, se debe elegir un plan (incluyendo "sin financiación" = 0)
  }
  return true;
}, {
